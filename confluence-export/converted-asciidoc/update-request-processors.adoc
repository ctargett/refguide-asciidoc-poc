...
This can be done by adding either "update.chain" or "processor" and "post-processor" as default parameter for a given path which can be done either via <<initparams-in-solrconfig.adoc,InitParams in SolrConfig>> or by adding them in a link:REL_LINK//requesthandlers-and-searchcomponents-in-solrconfig.adoc["defaults" section] which is supported by all request handlers.

...
The first is __link:#UpdatingPartsofDocuments-AtomicUpdates[atomic updates]__. This approach allows changing only one or more fields of a document without having to re-index the entire document.

The second approach is known as _link:#UpdatingPartsofDocuments-OptimisticConcurrency[optimistic concurrency]_ or __optimistic locking__. It is a feature of many NoSQL databases, and allows conditional updating a document based on its version. This approach includes semantics and rules for how to deal with version matches or mis-matches.

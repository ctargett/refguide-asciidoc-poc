...
The query issued to the system was __Solr__. It seems clear that faceting could not yield a similar set of groups, although the goals of both techniques are similar—to let the user explore the set of search results and either rephrase the query or narrow the focus to a subset of current documents. Clustering is also similar to link:Result-Grouping.html[Result Grouping] in that it can help to look deeper into search results, beyond the top few hits.

Topics covered in this section:

* link:#ResultClustering-PreliminaryConcepts[Preliminary Concepts]
* link:#ResultClustering-QuickStartExample[Quick Start Example]
* link:#ResultClustering-Installation[Installation]
* link:#ResultClustering-Configuration[Configuration]
* link:#ResultClustering-TweakingAlgorithmSettings[Tweaking Algorithm Settings]
* link:#ResultClustering-PerformanceConsiderations[Performance Considerations]
* link:#ResultClustering-AdditionalResources[Additional Resources]

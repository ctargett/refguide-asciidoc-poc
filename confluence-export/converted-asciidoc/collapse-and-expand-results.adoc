...
|min | max a|
Selects the group head document for each group based on which document has the min or max value of the specified numeric field or link:function-queries.html[function query].
...
If none are specified, the group head document of each group will be selected based on the highest scoring document in that group.
 ...
 |none
|sort a|
Selects the group head document for each group based on which document comes first according to the specified link:common-query-parameters.html#CommonQueryParameters-ThesortParameter[sort string].

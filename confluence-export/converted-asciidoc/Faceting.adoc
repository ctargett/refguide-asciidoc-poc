...
|link:#Faceting-ThefacetParameter[facet] |If set to true, enables faceting.
|link:#Faceting-Thefacet.queryParameter[facet.query] |Specifies a Lucene query to generate a facet count.
|========================================================================================================
...
|========================================================================================================================================================================================================================================================================
|Parameter |Description
|link:#Faceting-Thefacet.fieldParameter[facet.field] |Identifies a field to be treated as a facet.
|link:#Faceting-Thefacet.prefixParameter[facet.prefix] |Limits the terms used for faceting to those that begin with the specified prefix.
|link:#Faceting-Thefacet.containsParameter[facet.contains] |Limits the terms used for faceting to those that contain the specified substring.
|link:#Faceting-Thefacet.contains.ignoreCaseParameter[facet.contains.ignoreCase] |If facet.contains is used, ignore case when searching for the specified substring.
|link:#Faceting-Thefacet.sortParameter[facet.sort] |Controls how faceted results are sorted.
|link:#Faceting-Thefacet.limitParameter[facet.limit] |Controls how many constraints should be returned for each facet.
|link:#Faceting-Thefacet.offsetParameter[facet.offset] |Specifies an offset into the facet results at which to begin displaying facets.
|link:#Faceting-Thefacet.mincountParameter[facet.mincount] |Specifies the minimum counts required for a facet field to be included in the response.
|link:#Faceting-Thefacet.missingParameter[facet.missing] |Controls whether Solr should compute a count of all matching results which have no value for the field, in addition to the term-based constraints of a facet field.
|link:#Faceting-Thefacet.methodParameter[facet.method] |Selects the algorithm or method Solr should use when faceting a field.
|link:#Faceting-Thefacet.enum.cache.minDfParameter[facet.enum.cache.minDF] |(Advanced) Specifies the minimum document frequency (the number of documents matching a term) for which the `filterCache` should be used when determining the constraint count for that term.
|link:#Faceting-Over-RequestParameters[facet.overrequest.count] |(Advanced) A number of documents, beyond the effective `facet.limit` to request from each shard in a distributed search
| link:#Faceting-Over-RequestParameters[facet.overrequest.ratio] |(Advanced) A multiplier of the effective `facet.limit` to request from each shard in a distributed search
|link:#Faceting-Thefacet.threadsParameter[facet.threads] |(Advanced) Controls parallel execution of field faceting
|========================================================================================================================================================================================================================================================================
...
You can use Range Faceting on any date field or any numeric field that supports range queries. This is particularly useful for stitching together a series of range queries (as facet by query) for things like prices. As of Solr 3.1, Range Faceting is preferred over link:#Faceting-DateFacetingParameters[Date Faceting] (described below).

[width="100%",cols="50%,50%",options="header",]
|=========================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================
|Parameter |Description
|link:#Faceting-Thefacet.rangeParameter[facet.range] |Specifies the field to facet by range.
|link:#Faceting-Thefacet.range.startParameter[facet.range.start] |Specifies the start of the facet range.
|link:#Faceting-Thefacet.range.endParameter[facet.range.end] |Specifies the end of the facet range.
|link:#Faceting-Thefacet.range.gapParameter[facet.range.gap] |Specifies the span of the range as a value to be added to the lower bound.
|link:#Faceting-Thefacet.range.hardendParameter[facet.range.hardend] |A boolean parameter that specifies how Solr handles a range gap that cannot be evenly divided between the range start and end values. If true, the last range constraint will have the `facet.range.end` value an upper bound. If false, the last range will have the smallest possible upper bound greater then `facet.range.end` such that the range is the exact width of the specified range gap. The default value for this parameter is false.
|link:#Faceting-Thefacet.range.includeParameter[facet.range.include] |Specifies inclusion and exclusion preferences for the upper and lower bounds of the range. See the `facet.range.include` topic for more detailed information.
|link:#Faceting-Thefacet.range.otherParameter[facet.range.other] |Specifies counts for Solr to compute in addition to the counts for each facet range constraint.
|link:#Faceting-Thefacet.range.methodParameter[facet.range.method] |Specifies the algorithm or method to use for calculating facets.
|=========================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================
...
The span of each range expressed as a value to be added to the lower bound. For date fields, this should be expressed using the http://lucene.apache.org/solr/5_3_0/solr-core/org/apache/solr/util/DateMathParser.html[`DateMathParser` syntax] (such as, `facet.range.gap=%2B1DAY ... '+1DAY'`). You can specify this parameter on a per-field basis with the syntax of `f.<fieldname>.facet.range.gap`. For example:
...
Range faceting on date fields is a common situation where the link:Working-with-Dates.html#WorkingwithDates-TZ[`TZ`] parameter can be useful to ensure that the "facet counts per day" or "facet counts per month" are based on a meaningful definition of when a given day/month "starts" relative to a particular TimeZone.

For more information, see the examples in the link:Working-with-Dates.html[Working with Dates] section.

[[Faceting-DateFacetingParameters]]
Date Faceting Parameters
~~~~~~~~~~~~~~~~~~~~~~~~

Date faceting using the type specific `facet.date` parameters has been deprecated since Solr 3.1. Existing users are encouraged to switch to using the more general link:#Faceting-RangeFaceting[Range Faceting], which provides the same features for date fields, but can also work with any numeric field.
...
In addition to some of the link:#Faceting-LocalParametersforFaceting[general local parameters] supported by other types of faceting, a `stats` local parameters can be used with `facet.pivot` to refer to ` stats.field ` instances (by tag) that you would like to have computed for each Pivot Constraint.
...
* ` facet.limit `
* ` facet.offset `
* link:#Faceting-Thefacet.sortParameter[`facet.sort`]
...
|====================================================================================================
|*Name* |*What it does*
|link:#Faceting-Thefacet.intervalparameter[facet.interval] |Specifies the field to facet by interval.
|link:#Faceting-Thefacet.interval.setparameter[facet.interval.set] |Sets the intervals for the field.
|====================================================================================================
...
The link:Local-Parameters-in-Queries.html[LocalParams syntax] allows overriding global settings. It can also provide a method of adding metadata to other parameter values, much like XML attributes.
...
* link:Spatial-Search.html[Heatmap Faceting (Spatial)]

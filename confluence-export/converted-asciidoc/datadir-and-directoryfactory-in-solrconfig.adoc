...
If you are using Hadoop and would like to store your indexes in HDFS, you should use the `solr.HdfsDirectoryFactory` instead of either of the above implementations. For more details, see the section link:running-solr-on-hdfs.html[Running Solr on HDFS].

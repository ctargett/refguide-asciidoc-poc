...
* `/config/params` : allows creating parameter sets that can override or take the place of parameters defined in `solrconfig.xml`. See the link:request-parameters-api.html[Request Parameters API] section for more details.
...
* link:#ConfigAPI-CommandsforCustomHandlersandLocalComponents[Components]

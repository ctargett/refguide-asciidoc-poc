...
* `SolrClient.shutdown()` has been removed, use http://lucene.apache.org/solr/6_1_0/solr-solrj/org/apache/solr/client/solrj/SolrClient.html#close()[`SolrClient.close()`] instead.
...
Users upgrading from older versions are strongly encouraged to consult http://lucene.apache.org/solr/6_1_0/changes/Changes.html[`CHANGES.txt`] for the details of _all_ changes since the version they are upgrading from.

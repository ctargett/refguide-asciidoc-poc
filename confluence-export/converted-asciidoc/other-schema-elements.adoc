...
Although they have been deprecated for quite some time, Solr still has support for Schema based configuration of a `<defaultSearchField/>` (which is superseded by the `df parameter`) and `<solrQueryParser defaultOperator="OR"/>` (which is superseded by the link:the-standard-query-parser.html[`q.op` parameter].

If you have these options specified in your Schema, you are strongly encouraged to replace them with request parameters (or link:request-parameters-api.html[request parameter defaults]) as support for them may be removed from future Solr release.

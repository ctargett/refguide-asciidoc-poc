...
This filter creates tokens using the ` DoubleMetaphone ` encoding algorithm from commons-codec. For more information, see the <<phonetic-matching.adoc,Phonetic Matching>> section.
...
This is specialized version of the <<#FilterDescriptions-StopFilter,Stop Words Filter Factory>> that uses a set of stop words that are link:REL_LINK//managed-resources.adoc[managed from a REST API.]
...
This is specialized version of the <<#FilterDescriptions-SynonymFilter,Synonym Filter Factory>> that uses a mapping on synonyms that is link:REL_LINK//managed-resources.adoc[managed from a REST API.]

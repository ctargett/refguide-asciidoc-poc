...
Solr's `TrieDateField` (and deprecated `DateField`) represents a point in time with millisecond precision. The format used is a restricted form of the canonical representation of `dateTime` in the http://www.w3.org/TR/xmlschema-2/#dateTime[XML Schema specification]:
...
These are valid queries:datefield:1972-05-20T17\:33\:18.772Zdatefield:"1972-05-20T17:33:18.772Z"datefield:[1972-05-20T17:33:18.772 TO *]
...
By default, all date math expressions are evaluated relative to the UTC TimeZone, but the `TZ` parameter can be specified to override this behaviour, by forcing all date based addition and rounding to be relative to the specified http://docs.oracle.com/javase/7/docs/api/java/util/TimeZone.html[time zone].

...
* link:#OtherParsers-GraphQueryParser[Graph Query Parser]
...
* link:#OtherParsers-XMLQueryParser[XML Query Parser]
...
The `graph` parser only works in single node Solr installations, or with link:solrcloud.html[SolrCloud] collections that use exactly 1 shard.
...
Details about using the `ReRankQParserPlugin` can be found in the link:query-re-ranking.html[Query Re-Ranking] section.

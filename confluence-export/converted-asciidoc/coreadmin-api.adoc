...
The Core Admin API is primarily used under the covers by the link:collections-api.html[Collections API] when running a link:solrcloud.html[SolrCloud] cluster. SolrCloud users should not typically use the CoreAdmin API directly – but it may be useful for users of single-node or master/slave Solr installations for core maintenance operations.

...
The Core Admin API is primarily used under the covers by the <<collections-api.adoc,Collections API>> when running a link:REL_LINK//solrcloud.adoc[SolrCloud] cluster. SolrCloud users should not typically use the CoreAdmin API directly – but it may be useful for users of single-node or master/slave Solr installations for core maintenance operations.

...
To begin, you need to define a field type that uses the link:Filter-Descriptions.html#FilterDescriptions-ManagedStopFilter[ManagedStopFilterFactory], such as:
...
The `sample_techproducts_configs` link:Config-Sets.html[config set] ships with a pre-built set of managed stop words, however you should only interact with this file using the API and not edit it directly.
...
For the most part, the API for managing synonyms behaves similar to the API for stop words, except instead of working with a list of words, it uses a map, where the value for each entry in the map is a set of synonyms for a term. As with stop words, the `sample_techproducts_configs` link:Config-Sets.html[config set] includes a pre-built set of synonym mappings suitable for the sample data that is activated by the following field type definition in schema.xml:
...
As of Solr 5.2, you can also PUT a list of symmetric synonyms, which will be expanded into a mapping for each term in the list. For example, you could PUT the following list of symmetric synonyms using the JSON list syntax instead of a map:
...
* http://searchhub.org/2014/03/31/introducing-solrs-restmanager-and-managed-stop-words-and-synonyms/[Using Solr’s REST APIs to manage stop words and synonyms] by Tim Potter @ SearchHub.org

...
Solr's <<schema-api.adoc,Schema API>> enables remote clients to access link:REL_LINK//documents-fields-and-schema-design.adoc[Schema] information, and make Schema modifications, through a REST interface. Other features such as Solr's link:REL_LINK//schemaless-mode.adoc[Schemaless Mode] also work via Schema modifications made programatically at run time.

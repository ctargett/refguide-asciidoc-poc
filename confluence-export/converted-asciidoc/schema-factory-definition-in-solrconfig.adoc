...
Solr's link:schema-api.html[Schema API] enables remote clients to access link:documents-fields-and-schema-design.html[Schema] information, and make Schema modifications, through a REST interface. Other features such as Solr's link:schemaless-mode.html[Schemaless Mode] also work via Schema modifications made programatically at run time.
...
With the default configuration shown above, you can use the link:schema-api.html[Schema API] to modify the schema as much as you want, and then later change the value of `mutable` to *false* if you wish to "lock" the schema in place and prevent future changes.

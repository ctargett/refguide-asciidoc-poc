...
While one of the advantages of using SolrCloud is the ability to query very large collections distributed among various shards, in some cases link:shards-and-indexing-data-in-solrcloud.html#ShardsandIndexingDatainSolrCloud-DocumentRouting[you may know that you are only interested in results from a subset of your shards]. You have the option of searching over all of your data or just parts of it.

...
Result Grouping is separate from link:faceting.html[Faceting]. Though it is conceptually similar, faceting returns all relevant results and allows the user to refine the results based on the facet category. For example, if you search for "shoes" on a footwear retailer's e-commerce site, Solr would return all results for that query term, along with selectable facets such as "size," "color," "brand," and so on.
...
*NOTE:* This option does not work with link:#ResultGrouping-DistributedResultGroupingCaveats[distributed searches].
...
See below for link:#ResultGrouping-DistributedResultGroupingCaveats[Distributed Result Grouping Caveats] when using sharded indexes
...
Grouping is supported for link:solrcloud.html[distributed searches], with some caveats:

* Currently `group.func` is is not supported in any distributed searches
* `group.ngroups` and `group.facet` require that all documents in each group must be co-located on the same shard in order for accurate counts to be returned. link:shards-and-indexing-data-in-solrcloud.html[Document routing via composite keys] can be a useful solution in many situations.

...
In addition to some of the <<Faceting-LocalParametersforFaceting,general local parameters>> supported by other types of faceting, a `stats` local parameters can be used with `facet.pivot` to refer to <<the-stats-component.adoc#,`stats.field`>> instances (by tag) that you would like to have computed for each Pivot Constraint.
...
* <<Faceting-Thefacet.limitParameter,`facet.limit`>>
* <<Faceting-Thefacet.offsetParameter,`facet.offset`>>
* <<Faceting-Thefacet.sortParameter,`facet.sort`>>
* <<Faceting-Over-RequestParameters,`facet.overrequest.count`>>
* <<Faceting-Over-RequestParameters,`facet.overrequest.ratio`>>

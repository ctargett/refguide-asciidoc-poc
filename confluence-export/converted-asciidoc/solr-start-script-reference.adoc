...
Each example launches Solr in with a managed schema, which allows use of the link:schema-api.html[Schema API] to make schema edits, but does not allow manual editing of a Schema file If you would prefer to manually modify a `schema.xml` file directly, you can change this default as described in the section link:schema-factory-definition-in-solrconfig.html[Schema Factory Definition in SolrConfig].

Unless otherwise noted in the descriptions below, the examples do not enable link:solrcloud.html[SolrCloud] nor link:schemaless-mode.html[schemaless mode].
...
This command does *not* automatically make changes effective! It simply uploads the configuration sets to Zookeeper. You can use the link:collections-api.html[Collections API] to issue a RELOAD command for any collections that uses this configuration set.

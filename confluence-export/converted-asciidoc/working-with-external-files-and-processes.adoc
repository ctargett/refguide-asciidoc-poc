...
* link:#WorkingwithExternalFilesandProcesses-JsonPreAnalyzedParser[JsonPreAnalyzedParser]: as the name suggests, it parses content that uses JSON to represent field's content. This is the default parser to use if the field type is not configured otherwise.
* link:#WorkingwithExternalFilesandProcesses-SimplePreAnalyzedParser[SimplePreAnalyzedParser]: uses a simple strict plain text format, which in some situations may be easier to create than JSON.

...
* link:#RequestDispatcherinSolrConfig-handleSelectElement[handleSelect Element]
* link:#RequestDispatcherinSolrConfig-requestParsersElement[requestParsers Element]
* link:#RequestDispatcherinSolrConfig-httpCachingElement[httpCaching Element]
...
In recent versions of Solr, a `/select` requestHandler is defined by default, so a value of "false" will work fine. See the section link:RequestHandlers-and-SearchComponents-in-SolrConfig.html[RequestHandlers and SearchComponents in SolrConfig] for more information.

...
Like all link:streaming-expressions.html[Streaming Expressions], you can execute a `gatherNodes` expression by sending it to the `/stream` handler. For example:
...
In the example above only emails that match the filter query will be included in the traversal. Any Solr query can be included here. So you can do fun things like link:spatial-search.html[geospatial queries], apply any of the available link:query-syntax-and-parsing.html[query parsers], or even write custom query parsers to limit the traversal.

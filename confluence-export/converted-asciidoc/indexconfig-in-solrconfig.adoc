...
Controls whether newly written (and not yet merged) index segments should use the link:#IndexConfiginSolrConfig-CompoundFileSegments[Compound File Segment] format. The default is false.
...
To configure whether _newly written segments_ should use CFS, see the link:#IndexConfiginSolrConfig-useCompoundFile[`useCompoundFile`] setting described above. To configure whether _merged segments_ use CFS, review the Javadocs for your `mergePolicyFactory`.

Many link:#IndexConfiginSolrConfig-MergingIndexSegments[Merge Policy] implementations support `noCFSRatio` and `maxCFSSegmentSizeMB` settings with default values that prevent compound files from being used for large segments, but do use compound files for small segments.

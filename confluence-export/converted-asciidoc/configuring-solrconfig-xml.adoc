...
* link:schema-factory-definition-in-solrconfig.html[Schema Factory Definition in SolrConfig]
...
* link:codec-factory.html[Codec Factory]
...
The link:config-api.html[Config API] allows you to use an API to modify Solr's configuration, specifically user defined properties. Changes made with this API are stored in a file named `configoverlay.json`. This file should only be edited with the API, but will look like this example:
...
For more details, see the section link:config-api.html[Config API].

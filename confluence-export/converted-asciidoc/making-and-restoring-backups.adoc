...
Support for backups when running SolrCloud is provided with the link:collections-api.html[Collections API]. This allows the backups to be generated across multiple shards, and restored to the same number of shards and replicas as the original collection.

Two commands are available:

* `action=BACKUP`: This command backs up Solr indexes and configurations. More information is available in the section link:collections-api.html#CollectionsAPI-BackupCollection[Backup Collection].
* `action=RESTORE`: This command restores Solr indexes and configurations. More information is available in the section link:collections-api.html#CollectionsAPI-RestoreCollection[Restore Collection].

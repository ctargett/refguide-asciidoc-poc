...
These components are considered experimental, and must be explicitly enabled for a request handler in `solrconfig.xml`, in the same way as any other link:requesthandlers-and-searchcomponents-in-solrconfig.html[search component].
...
Documents should be added in children-parent blocks as described in link:uploading-data-with-index-handlers.html#UploadingDatawithIndexHandlers-NestedChildDocuments[indexing nested child documents]. Examples:
...
Queries are constructed the same way as for a link:other-parsers.html#OtherParsers-BlockJoinQueryParsers[Parent Block Join query]. For example:

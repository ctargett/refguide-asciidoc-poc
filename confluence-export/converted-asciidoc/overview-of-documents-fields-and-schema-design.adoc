...
Field analysis is an important part of a field type. link:understanding-analyzers-tokenizers-and-filters.html[Understanding Analyzers, Tokenizers, and Filters] is a detailed description of field analysis.

[[OverviewofDocuments,Fields,andSchemaDesign-Solr'sSchemaFile]]
== Solr's Schema File
...
* `managed-schema` is the name for the schema file Solr uses by default to support making Schema changes at runtime via the link:schema-api.html[Schema API], or link:schemaless-mode.html[Schemaless Mode] features. You may link:schema-factory-definition-in-solrconfig.html[explicitly configure the managed schema features] to use an alternative filename if you choose, but the contents of the files are still updated automatically by Solr.
...
* If you are using SolrCloud you may not be able to find any file by these names on the local filesystem. You will only be able to see the schema through the Schema API (if enabled) or through the Solr Admin UI's link:cloud-screens.html[Cloud Screens].
...
Note that if you are not using the Schema API yet you do use SolrCloud, you will need to interact with `schema.xml` through ZooKeeper using upconfig and downconfig commands to make a local copy and upload your changes. The options for doing this are described in link:solr-start-script-reference.html[Solr Start Script Reference] and link:using-zookeeper-to-manage-configuration-files.html[Using ZooKeeper to Manage Configuration Files].

...
* link:#FilterDescriptions-ASCIIFoldingFilter[ASCII Folding Filter]
* link:#FilterDescriptions-Beider-MorseFilter[Beider-Morse Filter]
* link:#FilterDescriptions-ClassicFilter[Classic Filter]
* link:#FilterDescriptions-CommonGramsFilter[Common Grams Filter]
* link:#FilterDescriptions-CollationKeyFilter[Collation Key Filter]
* link:#FilterDescriptions-Daitch-MokotoffSoundexFilter[Daitch-Mokotoff Soundex Filter]
* link:#FilterDescriptions-DoubleMetaphoneFilter[Double Metaphone Filter]
* link:#FilterDescriptions-EdgeN-GramFilter[Edge N-Gram Filter]
* link:#FilterDescriptions-EnglishMinimalStemFilter[English Minimal Stem Filter]
* link:#FilterDescriptions-HunspellStemFilter[Hunspell Stem Filter]
* link:#FilterDescriptions-HyphenatedWordsFilter[Hyphenated Words Filter]
* link:#FilterDescriptions-ICUFoldingFilter[ICU Folding Filter]
* link:#FilterDescriptions-ICUNormalizer2Filter[ICU Normalizer 2 Filter]
* link:#FilterDescriptions-ICUTransformFilter[ICU Transform Filter]
* link:#FilterDescriptions-KeepWordsFilter[Keep Words Filter]
* link:#FilterDescriptions-KStemFilter[KStem Filter]
* link:#FilterDescriptions-LengthFilter[Length Filter]
* link:#FilterDescriptions-LowerCaseFilter[Lower Case Filter]
* link:#FilterDescriptions-ManagedStopFilter[Managed Stop Filter]
* link:#FilterDescriptions-ManagedSynonymFilter[Managed Synonym Filter]
* link:#FilterDescriptions-N-GramFilter[N-Gram Filter]
* link:#FilterDescriptions-NumericPayloadTokenFilter[Numeric Payload Token Filter]
* link:#FilterDescriptions-PatternReplaceFilter[Pattern Replace Filter]
* link:#FilterDescriptions-PhoneticFilter[Phonetic Filter]
* link:#FilterDescriptions-PorterStemFilter[Porter Stem Filter]
* link:#FilterDescriptions-RemoveDuplicatesTokenFilter[Remove Duplicates Token Filter]
* link:#FilterDescriptions-ReversedWildcardFilter[Reversed Wildcard Filter]
* link:#FilterDescriptions-ShingleFilter[Shingle Filter]
* link:#FilterDescriptions-SnowballPorterStemmerFilter[Snowball Porter Stemmer Filter]
* link:#FilterDescriptions-StandardFilter[Standard Filter]
* link:#FilterDescriptions-StopFilter[Stop Filter]
* link:#FilterDescriptions-SuggestStopFilter[Suggest Stop Filter]
* link:#FilterDescriptions-SynonymFilter[Synonym Filter]
* link:#FilterDescriptions-TokenOffsetPayloadFilter[Token Offset Payload Filter]
* link:#FilterDescriptions-TrimFilter[Trim Filter]
* link:#FilterDescriptions-TypeAsPayloadFilter[Type As Payload Filter]
* link:#FilterDescriptions-TypeTokenFilter[Type Token Filter]
* link:#FilterDescriptions-WordDelimiterFilter[Word Delimiter Filter]
* link:#FilterDescriptions-RelatedTopics[Related Topics]
...
Implements the Beider-Morse Phonetic Matching (BMPM) algorithm, which allows identification of similar names, even if they are spelled differently or in different languages. More information about how this works is available in the section on link:Phonetic-Matching.html[Phonetic Matching].
...
Implements the Daitch-Mokotoff Soundex algorithm, which allows identification of similar names, even if they are spelled differently. More information about how this works is available in the section on link:Phonetic-Matching.html[Phonetic Matching].
...
`dictionary`: (required) The path of a dictionary file. `affix`: (required) The path of a rules file. `ignoreCase`: (boolean) controls whether matching is case sensitive or not. The default is false. `strictAffixParsing`: (boolean) controls whether the affix parsing is strict or not. If true, an error while reading an affix rule causes a ParseException, otherwise is ignored. The default is true.
...
[[FilterDescriptions-KeepWordsFilter]]
Keep Words Filter
~~~~~~~~~~~~~~~~~
...
See link:#FilterDescriptions-StopFilter[Stop Filter] for example input/output.

[[FilterDescriptions-ManagedSynonymFilter]]
Managed Synonym Filter
~~~~~~~~~~~~~~~~~~~~~~

This is specialized version of the link:#FilterDescriptions-SynonymFilter[Synonym Filter Factory] that uses a mapping on synonyms that is link:Managed-Resources.html[managed from a REST API.]
...
See link:#FilterDescriptions-SynonymFilter[Synonym Filter] for example input/output.
...
Like link:#FilterDescriptions-StopFilter[Stop Filter], this filter discards, or _stops_ analysis of, tokens that are on the given stop words list. Suggest Stop Filter differs from Stop Filter in that it will not remove the last token unless it is followed by a token separator. For example, a query "`find the`" would preserve the '`the`' since it was not followed by a space, punctuation etc., and mark it as a `KEYWORD` so that following filters will not change or remove it. By contrast, a query like "`find the popsicle`" would remove "`the`" as a stopword, since it's followed by a space. When using one of the analyzing suggesters, you would normally use the ordinary `StopFilterFactory` in your index analyzer and then SuggestStopFilter in your query analyzer.
...
This filter blacklists or whitelists a specified list of token types, assuming the tokens have type metadata associated with them. For example, the link:Tokenizers.html#Tokenizers-UAX29URLEmailTokenizer[UAX29 URL Email Tokenizer] emits "<URL>" and "<EMAIL>" typed tokens, as well as other types. This filter would allow you to pull out only e-mail addresses from text as tokens, if you wish.
...
[[FilterDescriptions-RelatedTopics]]
Related Topics
~~~~~~~~~~~~~~

* http://wiki.apache.org/solr/AnalyzersTokenizersTokenFilters#TokenFilterFactories[TokenFilterFactories]

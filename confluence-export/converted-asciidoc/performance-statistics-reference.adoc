...
This page will explain all the link:using-jmx-with-solr.html[JMX] statistics that Solr exposes. The same stats are also exposed via thelink:mbean-request-handler.html[MBean Request Handler] when stats are requested.
...
More on link:query-settings-in-solrconfig.html[Solr caches]

...
This page will explain all the <<using-jmx-with-solr.adoc#,JMX>> statistics that Solr exposes. The same stats are also exposed via the<<mbean-request-handler.adoc#,MBean Request Handler>> when stats are requested.

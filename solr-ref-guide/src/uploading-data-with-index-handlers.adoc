...
Note that if you are not working in <<schemaless-mode.adoc#schemaless-mode,Schemaless Mode>>, where fields that don't exist will be created on the fly with Solr's best guess for the field type, documents may get rejected if the fields do not exist in the schema before indexing.

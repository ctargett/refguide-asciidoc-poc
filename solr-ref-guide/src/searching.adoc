...
* <<highlighting.adoc#highlighting,Highlighting>>: Detailed information about Solr's highlighting utilities. Sub-sections cover the different types of highlighters:
** <<standard-highlighter.adoc#standard-highlighter,Standard Highlighter>>: Uses the most sophisticated and fine-grained query representation of the three highlighters.
** <<fastvector-highlighter.adoc#fastvector-highlighter,FastVector Highlighter>>: Optimized for term vector options on fields, and good for large documents and multiple languages.
** <<postings-highlighter.adoc#postings-highlighter,Postings Highlighter>>: Uses similar options as the FastVector highlighter, but is more compact and efficient.

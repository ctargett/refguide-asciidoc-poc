...
Please follow the instructions to install Solr as a Service on Linux documented at <<taking-solr-to-production.adoc#taking-solr-to-production,Taking Solr to Production>>. After running the install script, the new Solr node will be running, so please stop it by doing: `sudo service solr stop`. You need to update the `/etc/default/solr.in.sh` include file in the next step to complete the upgrade process.

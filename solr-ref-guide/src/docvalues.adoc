...
If `docValues="true"` for a field, then DocValues will automatically be used any time the field is used for <<common-query-parameters.adoc#CommonQueryParameters-ThesortParameter,sorting>>, https://cwiki.apache.org/confluence/display/solr/Faceting[faceting] or <<function-queries.adoc#function-queries,Function Queries>>.

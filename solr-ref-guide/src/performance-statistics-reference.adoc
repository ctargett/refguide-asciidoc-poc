...
This page will explain all the <<using-jmx-with-solr.adoc#using-jmx-with-solr,JMX>> statistics that Solr exposes. The same stats are also exposed via the<<mbean-request-handler.adoc#mbean-request-handler,MBean Request Handler>> when stats are requested.
...
More on <<query-settings-in-solrconfig.adoc#query-settings-in-solrconfig,Solr caches>>
